...
public abstract class AbstractHWObsClient {
    ...
    public boolean isBucketExist() throws IOException {
        Request.Builder requestBuilder = new Request.Builder()
                .url(generateUrl())
                .head();
        String canonicalResource = generateCanonicalResource();
        appendDateAndSignRequest(requestBuilder, "HEAD", canonicalResource);
     ...
     * @param date    时间信息
     */
    protected void appendDate(Request.Builder builder, String date) {
     ...
     */
    protected void appendDateAndSignRequest(Request.Builder builder, String method, String canonicalResource) {
        String requestTime = getDate();
        appendDate(builder, requestTime);

        String canonicalString = generateCanonicalString(method,
                "", "", requestTime, "",
                canonicalResource);
     ...
     */
    protected void signRequest(Request.Builder builder, String canonicalString) {
        String signature = HuaweiSignature.signWithHmacSha1(getSecurityKey(), canonicalString);
        builder.addHeader("Authorization", "OBS " + getAccessKey() + ":" + signature);
    }
    ...
    protected String readResponse(Response response) throws IOException {
        ...
        if (body != null) {
            try (BufferedInputStream bis = new BufferedInputStream(body.byteStream())) {
                int inputChar;
     ...
     */
    protected String requireValidObjectKey(String objectKey) {
        ...
        objectKey = UriUtils.encode(objectKey, "UTF-8");
     ...
     * @param response 响应
     */
    protected void outputHeader(Request request, Response response) {
        System.out.println("request message");
        System.out.println(request);
        System.out.println(request.headers());
